...
int parseIntegerOption(QCommandLineParser* parser,
                       const QCommandLineOption& option, int defaultValue,
...
QHostAddress parseBindAddress(QCommandLineParser* parser,
                              const QCommandLineOption& option,
                              const QString& defaultValue) {
  const QString rawValue =
      parser->isSet(option) ? parser->value(option).trimmed() : defaultValue;
  ...
  if (!address.setAddress(rawValue)) {
    qCritical().noquote() << QString(
                                 "Invalid --%1 value '%2'. Use an IP address, "
                                 "localhost, loopback, or any.")
                                 .arg(option.names().last(), rawValue);
    parser->showHelp(1);
  }
...
MainWindow::MainWindow(int argc, char* argv[], QWidget* parent)
  ...
  m_settings = new QSettings(QSettings::NativeFormat, QSettings::UserScope,
                             PIXYMON_COMPANY, PIXYMON_TITLE);
  m_console = new ConsoleWidget(this);
  m_video = new VideoWidget(this);
  connect(m_video, SIGNAL(mouseLoc(int, int)), this,
          SLOT(handleMouseLoc(int, int)));
...
MainWindow::~MainWindow() {
  ...
  // we don't delete any of the widgets because the parent deletes it's children
  // upon deletion
...
void MainWindow::parseCommandline(int argc, char* argv[]) {
  ...
  parser.setApplicationDescription(
      "PixyMon with Pixy2 HTTP streaming support.");
  parser.setSingleDashWordOptionMode(QCommandLineParser::ParseAsLongOptions);
  parser.addHelpOption();
  parser.addVersionOption();

  QCommandLineOption firmwareOption("firmware", "Firmware file to upload.",
                                    "file");
  QCommandLineOption initScriptOption(
      "initscript", "Initialization script to run.", "script");
  ...
  QCommandLineOption bindOption("bind",
                                "HTTP bind address. Use 127.0.0.1 for "
                                "local-only or 0.0.0.0/any for LAN access.",
                                "address", k_defaultBindAddress);
  QCommandLineOption portOption("port", "HTTP server port.", "port",
                                QString::number(k_defaultServerPort));
  QCommandLineOption disableHttpOption(
      "disable-http", "Disable the HTTP snapshot/stream server.");
  ...
  QCommandLineOption qualityOption(
      "quality", "JPEG quality for snapshots and streams.", "quality",
      QString::number(k_defaultJpegQuality));
  QCommandLineOption maxClientsOption(
      "max-clients", "Maximum simultaneous HTTP clients.", "count",
      QString::number(k_defaultMaxHttpClients));
  QCommandLineOption requestTimeoutOption(
      "request-timeout-ms",
  ...
  m_httpBindAddress =
      parseBindAddress(&parser, bindOption, k_defaultBindAddress);
  m_httpPort =
      parseIntegerOption(&parser, portOption, k_defaultServerPort, 1, 65535);
  m_httpFps = parseIntegerOption(&parser, fpsOption, k_defaultStreamFps, 1, 60);
  m_httpJpegQuality =
      parseIntegerOption(&parser, qualityOption, k_defaultJpegQuality, 1, 100);
  m_httpMaxClients = parseIntegerOption(&parser, maxClientsOption,
                                        k_defaultMaxHttpClients, 1, 64);
  m_httpRequestTimeoutMs = parseIntegerOption(
      &parser, requestTimeoutOption, k_defaultRequestTimeoutMs, 1000, 60000);
...
void MainWindow::updateButtons() {
  ...
  if (m_configDialog) m_ui->actionConfigure->setEnabled(false);
}

void MainWindow::close() { QCoreApplication::exit(); }
...
void MainWindow::connectPixyDFU(bool state) {
  if (state)  // connect
  {
    ...
    if (!m_testCycle) showConsole(true);
    // close the instuctions dialog if it's open
    if (m_fwInstructions) m_fwInstructions->done(0);
    try {
      if (!m_pixyDFUConnected) {
        ...
        if (m_pixyflash == "") m_pixyflash = "pixyflash.bin.hdr";
        QString file = QFileInfo(path, m_pixyflash).absoluteFilePath();
        dfu->download(file);
        if (!m_testCycle) m_pixyDFUConnected = true;
        delete dfu;
        Sleeper::msleep(1000);
        m_connect = new ConnectEvent(this);
      }
...
void MainWindow::connectPixy(bool state) {
  if (state)  // connect
  {
    try {
      if (m_pixyDFUConnected)  // we're in programming mode
      {
        ...
        m_interpreter =
            new Interpreter(m_console, m_video, &m_parameters, m_initScript);

        connect(m_interpreter, SIGNAL(error(QString)), this,
                SLOT(error(QString)));
        connect(m_interpreter, SIGNAL(textOut(QString, uint)), this,
                SLOT(handleText(QString, uint)));
        connect(m_interpreter, SIGNAL(runState(int, QString)), this,
                SLOT(handleRunState(int, QString)));
        connect(m_interpreter, SIGNAL(finished()), this,
                SLOT(interpreterFinished()));  // thread will send finished
                                               // event when it exits
        connect(m_interpreter, SIGNAL(connected(Device, bool)), this,
                SLOT(handleConnected(Device, bool)));
        connect(m_interpreter,
                SIGNAL(actionScriptlet(QString, QStringList, bool)), this,
                SLOT(handleActionScriptlet(QString, QStringList, bool)));
        connect(m_interpreter, SIGNAL(view(QString, uint, bool, bool)), this,
                SLOT(handleView(QString, uint, bool, bool)));
        connect(m_interpreter, SIGNAL(prog(QString, QString, uint, bool)), this,
                SLOT(handleProg(QString, QString, uint, bool)));
        connect(m_interpreter, SIGNAL(paramLoaded()), this,
                SLOT(handleLoadParams()));
        connect(m_interpreter, SIGNAL(paramChange()), this,
                SLOT(handleParamChange()));
        connect(m_interpreter,
                SIGNAL(version(ushort, ushort, ushort, QString, ushort, ushort,
                               ushort)),
                this,
                SLOT(handleVersion(ushort, ushort, ushort, QString, ushort,
                                   ushort, ushort)));
        m_interpreter->start();
      }
  ...
  updateButtons();
  if (m_httpServer) m_httpServer->setInterpreter(m_interpreter);
}

void MainWindow::setEnabledActionsViews(bool enable) {
  uint i;

  for (i = 0; i < m_actions.size(); i++) m_actions[i]->setEnabled(enable);
}

void MainWindow::setEnabledProgs(bool enable) {
  uint i;

  for (i = 0; i < m_progs.size(); i++) m_progs[i]->setEnabled(enable);
}
...
void MainWindow::handleActionScriptlet(QString action, QStringList scriptlet,
  ...
  if (action != "") addAction(action, scriptlet);
}

void MainWindow::handleView(QString view, uint index, bool current,
                            bool reset) {
  uint i;
...
void MainWindow::handleProg(QString name, QString desc, uint index,
                            bool reset) {
  uint i;
...
void MainWindow::handleConnected(Device device, bool state) {
  ...
  // if we're incompatible, don't create/connect to device... wait for next
  // event
  if (m_versionIncompatibility) return;
...
void MainWindow::on_actionAbout_triggered() {
  ...
  contents.asprintf("<b>%s v2 version %d.%d.%d</b><br>", PIXYMON_TITLE,
  ...
  if (m_interpreter) {
    ...
    contents += fwver.asprintf("<b>Pixy2 firmware version %d.%d.%d ",
                               version[0], version[1], version[2]) +
                fwtype + " build</b> (queried)<br>";
    contents +=
        fwver.asprintf("<b>Pixy2 hardware version %d.%d.%d</b> (queried)<br>",
  ...
  contents +=
      ...
      "Please send problems, suggestions, ideas, inquiries and feedback, "
      "positive or negative "
      "(although we do love to hear the positive stuff!) "
      "to <a href=\"mailto:support@pixycam.com\">support@pixycam.com</a>. "
      "Additional information can be found on our <a "
      "href=\"http://pixycam.com/pixy2\">website</a> "
      "and our <a href=\"http://pixycam.com/pixywiki2\">documentation "
      "wiki</a>.<br><br>"
...
void MainWindow::on_actionPlay_Pause_triggered() {
  if (m_interpreter) m_interpreter->runOrStopProgram();
}

void MainWindow::on_actionDefault_program_triggered() {
  if (m_interpreter) m_interpreter->execute("runProgDefault");
}
...
void MainWindow::program(const QString& file) {
  ...
  // start looking for devices again (wait 4 seconds before we start looking
  // though)
  m_connect = new ConnectEvent(this, 4000);
}

void MainWindow::showConsole(bool show) {
  if (show == m_showConsole) return;

  if (show) {
    resize(width(),
           height() + m_console->height() + m_ui->imageLayout->spacing());
    m_console->show();
...
void MainWindow::on_actionConfigure_triggered() {
  if (m_interpreter) {
    ...
    connect(m_configDialog, SIGNAL(finished(int)), this,
            SLOT(handleConfigDialogFinished()));
...
void MainWindow::on_actionRaw_video_triggered() {
  if (m_interpreter) m_interpreter->execute("runProgName video");
}
...
void MainWindow::interpreterFinished() {
  ...
  if (m_httpServer) m_httpServer->setInterpreter(nullptr);
...
void MainWindow::handleFirmware(ushort major, ushort minor, ushort build,
                                const QString& type, ushort hwMajor,
                                ushort hwMinor, ushort hwBuild) {
  ...
  for (i = 0; i < files.size(); i++) {
    try {
      Flash::checkHardwareVersion(hwVersion,
                                  QFileInfo(path, files[i]).absoluteFilePath(),
                                  fwVersion, &fwType);
      ver = ((qlonglong)fwVersion[0] << 32) | ((qlonglong)fwVersion[1] << 16) |
            fwVersion[2];
      if (ver > currVer && fwType.compare(type, Qt::CaseInsensitive) == 0) {
        fwFilename = files[i];
        maxVerStr = QString::number(fwVersion[0]) + "." +
                    QString::number(fwVersion[1]) + "." +
                    QString::number(fwVersion[2]) + "-" + fwType;
      }
    } catch (std::runtime_error& exception) {
      // do nothing
    }
  ...
  if (m_fwMessage == NULL) {
    ...
    {
      QString str = "There is a more recent firmware version (" + maxVerStr +
                    ") available.<br>"
                    "Your Pixy is running firmware version " +
                    QString::number(major) + "." + QString::number(minor) +
                    "." + QString::number(build) + "-" + type +
                    ".<br><br>"
                    "Would you like to upload this firmware into your Pixy? "
                    "<i>Psst, click yes!</i>";
      m_fwMessage =
          new QMessageBox(QMessageBox::Question, "New firmware available!", str,
                          QMessageBox::Yes | QMessageBox::No);
      m_fwMessage->setDefaultButton(QMessageBox::Yes);
      m_fwMessage->exec();
      if (m_fwMessage->standardButton(m_fwMessage->clickedButton()) ==
          QMessageBox::Yes) {
        m_firmwareFile = QFileInfo(path, fwFilename).absoluteFilePath();
        m_fwInstructions = new QMessageBox(
            QMessageBox::Information, "Get your Pixy ready!",
            "Please follow these steps to get your Pixy ready to accept new "
            "firmware:<br><br>"
            "1. Unplug your Pixy from USB (do this now).<br>"
            "2. Press and hold down the button on top of Pixy.<br>"
            "3. Plug the USB cable back in while continuing to hold down the "
            "button.<br>"
            "4. Release the button after the USB cable is plugged back in.<br>"
            "5. Wait a bit. This window will go away automatically when all is "
            "ready<br>"
            "to proceed, but the drivers sometimes take a few minutes to "
            "install.");
    ...
    } else if (m_versionIncompatibility)  // otherwise tell them where to find
                                          // the latest version
    {
      m_fwMessage = new QMessageBox(
          QMessageBox::Information, "Firmware or hardware incompatibility",
          "This Pixy's firmware or hardware is incompatible with this version "
          "of PixyMon.<br>"
...
void MainWindow::handleVersion(ushort major, ushort minor, ushort build,
                               QString type, ushort hwMajor, ushort hwMinor,
                               ushort hwBuild) {
  QString str;

  if (major != VER_MAJOR || minor > VER_MINOR) {
    // Interpreter will automatically exit if there's a version incompatibility,
    // so no need to close interpreter
    m_versionIncompatibility = true;
  }
  handleFirmware(major, minor, build, type, hwMajor, hwMinor, hwBuild);
  // str = str.asprintf("Pixy firmware version %d.%d.%d.\n", major, minor,
  // build); m_console->print(str);
}
...
void MainWindow::on_actionHelp_triggered() {
  QString fwtype;
  if (m_interpreter) fwtype = m_interpreter->getVersionType();
...
void MainWindow::handleParamChange() {
  if (m_interpreter) m_interpreter->loadParams(true);
}
...
void MainWindow::on_actionSave_Image_triggered() {
  ...
  if (m_interpreter) {
    filename = uniqueFilename(m_parameters.value("Document folder").toString(),
                              "image", "png");
  ...
  if (m_interpreter) {
    ...
    if (fd.exec()) {
      ...
      if (flist.size() == 1) {
        ...
        res =
            pf.read(PIXY_PARAMFILE_TAG, &m_interpreter->m_pixyParameters, true);
        pf.close();
        ...
        if (res >= 0) {
          ...
          m_console->print(
              "Parameters have been successfully loaded!  Resetting...\n");
        }
      }
    }
  }
